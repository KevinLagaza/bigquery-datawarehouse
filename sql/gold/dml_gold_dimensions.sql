...
CREATE OR REPLACE VIEW `<project_id>.gold.dim_customers` AS
select 
...
from `<project_id>.silver.crm_cust_info` cci

    left join `<project_id>.silver.erp_cust_az12` eca
        on cci.cst_key = eca.cid

    left join `<project_id>.silver.erp_loc_a101` ela
...
CREATE OR REPLACE VIEW `<project_id>.gold.dim_products` AS
SELECT
...
from `<project_id>.silver.crm_prd_info` cpi

    left join `<project_id>.silver.erp_px_cat_g1v2` epc
